...
from mmseg.utils.utils import calc_mean_std
...
class MixVisionTransformer(BaseModule):
    ...
    def forward(self, x, zw=None):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if zw is not None:
                if i == 1:
                    xws = self.instance_norm_layer(outs[0], zw[0])
                    x = xws
                    outs[0] = x
                if i == 2:
                    xws = self.instance_norm_layer(outs[1], zw[1])
                    x = xws
            ################################
            #####################################
            ...
            if eliminate is not None:
                if i == 0:
                    size = x.size()
                    source_mean, source_std = calc_mean_std(x)
                
                    eliminated_x = (x - source_mean.expand(size))/source_std.expand(size)
                    x = eliminated_x
