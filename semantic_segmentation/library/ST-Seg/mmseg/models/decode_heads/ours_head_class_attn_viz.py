...
class OursHeadClassAtt(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        # 0: torch.Size([8, 32, 75, 94])
        # 1: torch.Size([8, 64, 38, 47])
        # 2: torch.Size([8, 160, 19, 24])
        # 3: torch.Size([8, 256, 10, 12])
        ...
        for idx in range(len(inputs)):
            ...
            outs.append(
                resize(
                    input=conv(x), # 1x1 conv 
                    size=inputs[self.in_index[self.size_index]].shape[2:],
                    mode='bilinear',
                    align_corners=self.align_corners))
        # config 의 channels 값으로 1x1 conv이용해서 맞춰준뒤 hw 통일시킨다.
        
        # 0: torch.Size([8, 384, 38, 47])
        # 1: torch.Size([8, 384, 38, 47])
        # 2: torch.Size([8, 384, 38, 47])
        # 3: torch.Size([8, 384, 38, 47])

        # channel 차원으로 concate 후에 1x1 conv 로 합치기 전 channel 수로 차원 축소
        x = self.fusion_conv(torch.cat(outs, dim=1))
        #x: torch.Size([8, 384, 38, 47])
        
        
        identity = x

        out, attn = self.attn(x)
        # out: torch.Size([8, 384, 38, 47])
        # attn: torch.Size([8, 6, 1786, 1786])
        
        align_corners = self.align_corners
        # from IPython import embed
        # embed()
        out = self.proj(out)
        # out: torch.Size([8, 64, 38, 47])
        ...
        attn = attn.permute(1, 0, 2, 3) # 6 8 1786 1786
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        ...
        # seg_logits:  torch.Size([8, 6, 38, 47])
        # att:  torch.Size([6, 8, 38, 47])
        losses = self.losses([seg_logits, att], gt_semantic_seg)
        # gt:  torch.Size([8, 1, 300, 375])
        return losses
