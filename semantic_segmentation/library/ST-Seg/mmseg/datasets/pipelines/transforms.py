...
class Wild_Normalize(object):
    ...
    def __call__(self, results):
        ...
        results['target_img'] = mmcv.imnormalize(results['target_img'], self.mean, self.std,
                                          self.to_rgb)
        results['img_norm_cfg'] = dict(
            mean=self.mean, std=self.std, to_rgb=self.to_rgb)
        return results
...
@PIPELINES.register_module()
class Wild_PhotoMetricDistortion(object):
    """Apply photometric distortion to image sequentially, every transformation
    is applied with a probability of 0.5. The position of random contrast is in
    second or second to last.
...
#         results['img'] = self.automold.augment_random(results['img'] )
#         mmcv.imwrite(results['img'], '/home/docker/workspace/mmseg/test.jpg')
#         fef
#         return results
