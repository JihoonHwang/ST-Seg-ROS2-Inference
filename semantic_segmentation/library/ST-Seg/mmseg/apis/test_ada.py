...
def single_gpu_our(model,
    ...
    param_list = []
    out_dir = "./cotta/weather/namyang900/"+str(datetime.datetime.now())
    ...
    for i, data in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            mask = (probs_[2][0] > 0.67).astype(np.int64) # 0.74 was the 5% quantile for cityscapes, therefore we use 0.69 here
            result = [(mask*preds[2][0] + (1.-mask)*result[0]).astype(np.int64)]
            weight = 1.
        if show or out_dir:
            img_tensor = data['img'][2]
            img_metas = data['img_metas'][2].data[0]
        ...
        if isinstance(result, list):
            ...
            #####
            result = model(return_loss=False, img=[data['img'][img_id]], img_metas=[data['img_metas'][img_id].data[0]])
            #####
